...
<div style='text-align:center;'>
  ...
  <p class='tip'>Connect to your Postgres database (Neon, Supabase, Railway, or any standard Postgres), initialize the schema, and create your first admin user.</p>
</div>
<hr/>
""", unsafe_allow_html=True)

# --- DB helpers (local to the wizard) ---
def _conn(url: str):
    # Force SSL if not specified (works for Neon/Supabase/Railway)
...
# --- Step 1: Connect to Postgres (Any provider) ---
st.subheader("Step 1 — Connect to Your Postgres Database")

with st.container():
    tab_neon, tab_existing = st.tabs(["Create a Free Neon DB", "Use an Existing Database"])

    with tab_neon:
        st.markdown("""
<div class='step'>
<b>Quick start with Neon (free tier)</b>
<ol>
<li>Go to <a href='https://neon.tech' target='_blank'>neon.tech</a> and sign in.</li>
...
""", unsafe_allow_html=True)
    ...
    with tab_existing:
        st.markdown("""
<div class='step'>
<b>Use any existing Postgres</b>
<ul>
<li><b>Supabase:</b> Project → Settings → Database → Connection string (URI)<br/>
<code>postgresql://postgres:YOUR-PASSWORD@YOUR-HOST.supabase.co:5432/postgres?sslmode=require</code></li>
<li><b>Railway:</b> Project → Postgres → Connect → Postgres Connection URL</li>
<li><b>Other:</b> Any standard Postgres URI with <code>sslmode=require</code> if hosted.</li>
</ul>
</div>
""", unsafe_allow_html=True)
...
if env_url:
    try:
        masked_host = env_url.split("@")[-1].split("?")[0]
    except Exception:
        masked_host = "(unknown)"

st.markdown("<div class='step'>", unsafe_allow_html=True)
st.markdown("**Current environment variable**")
if env_url:
    st.success(f"Found `DATABASE_URL` → `{masked_host}`")
else:
...
manual_url = st.text_input(
    ...
    value=env_url or "",
    type="password",
    help="Example: postgresql://user:password@host:5432/dbname?sslmode=require"
)
...
with colC:
    clear_clicked = st.button("Clear Saved Connection")

if test_clicked:
    try:
        with _conn(manual_url) as cn, cn.cursor() as c:
            c.execute("SELECT 1")
...
if clear_clicked:
    ...
    st.info("Cleared the saved connection for this session.")
...
st.markdown("<div class='step'>", unsafe_allow_html=True)
if not active_url:
    st.info("Paste your Postgres connection string above to continue.")
else:
    ...
    with col2:
        if st.button("Check Schema Status"):
            try:
                checks = {
                    "users": _table_exists(active_url, "users"),
                    "characters": _table_exists(active_url, "characters"),
                    "locations": _table_exists(active_url, "locations"),
                    "factions": _table_exists(active_url, "factions"),
                    "campaignevents": _table_exists(active_url, "campaignevents"),
                }
                for k, ok in checks.items():
                    st.write(f"- {k}: **{'OK' if ok else 'Missing'}**")
            except Exception as e:
